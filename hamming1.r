#This version of code works with multiple data files in a particular folder, 
#performs some matrix calculations on them and outputs the new matrices with their corresponding titles 
#into that same folder - currently there are 165 stateFiles within this particular folder which 
#means 330 additional files were created in that same folder in about a minutes worth of time. 

library(e1071) #Package that allows you to calculate hamming distance 

stateFiles <- list.files("/home/zackymo/Desktop/birth_data_zach_calcs_50", pattern="*state.txt", full.names=TRUE)
thres <- .5

for(file in stateFiles){
  rawData = read.table(file)
  df <- data.frame(rawData)
  
  #Creating the two matrices 
  incidence <- matrix(rep(0, ncol(df)*ncol(df)), nrow=ncol(df))
  thresholded <- matrix(rep(0, ncol(df)*ncol(df)), nrow=ncol(df)) 
  #hamming <- matrix(rep(0, ncol(df)*ncol(df)), nrow=ncol(df)) 
  
  #Set the diagonal = to constants due to the fact that both are symmetric matrices 
  diag(incidence) <- nrow(df)
  diag(thresholded) <- 1
  #diag(hamming) <- 0 prob unneccesary statement considering the whole hamming matrix already has 0's in it 
  
  #Describe for loop here 
  for (i in 1:(ncol(df)-1)) {
    for (j in (i+1):ncol(df)) {
      incidence[i,j] = incidence[j,i] = sum(df[,i] == df[,j])
      if(incidence[i,j]/nrow(df) >= thres){
        thresholded[i,j] = thresholded[j,i] = 1
      }#should these conditions be reversed for effiency? How can I check that? 
      else{
        thresholded[i,j] = thresholded[j,i] = 0
      }
      #hamming[i,j] = hamming[j,i] = sum(thresholded[,i] == thresholded[,j]) This should create hamming matrix
      #for each file -> should check if this is the same as using hamming.distance function 
    }
  }
 
  #These two methods work the same way, they compare the matrix row by row, evaluate whether or not rat 1 is chilling
  #with rat 2, if true add 1 to that [r,c], if false, add 0 
  #ENTER NEW STATISTICAL MECHANISMS HERE 
  hamming_loop <- function(X) {
    d <- matrix(0, nrow = nrow(X), ncol = nrow(X))
    for (i in 1:(nrow(X) - 1)) {
      for (j in (i + 1):nrow(X)) {
        d[i,j] <- d[j,i] <- sum(X[i,] != X[j,])
      }
    }
    d
  }
  hamming_loop(thresholded)
  
  #Here is the hamming.distance utilized in package e1071 - does the diagonal have to = 1 for this?
  #what you can do is create a hamming matrix for each file and then read in each hamming matrice the same way 
  #Using a list of hamming files and sum each one to come up with a total hamming matrix 
  #Where is Imberman getting her numbers from in regards to the spreadsheet? They're totals for each week which 
  #seems weird to me - Can create a program to put every hamming distance into a list and add each matrix from the particular 
  #hamming distance file 
  hamming1 <- hamming.distance(thresholded)
  
  """
  CODE THAT THE HAMMING.DISTANCE FUNCTION USES 

  getAnywhere(hamming.distance())
  A single object matching ‘hamming.distance’ was found
  It was found in the following places
  package:e1071
  namespace:e1071
  with value
  
function (x, y) 
  {
    z <- NULL
    if (is.vector(x) && is.vector(y)) {
      z <- sum(x != y)
    }
    else {
      z <- matrix(0, nrow = nrow(x), ncol = nrow(x))
        for (k in 1:(nrow(x) - 1)) {
          for (l in (k + 1):nrow(x)) {
            z[k, l] <- hamming.distance(x[k, ], x[l, ])
            z[l, k] <- z[k, l]
          }
        }
      dimnames(z) <- list(dimnames(x)[[1]], dimnames(x)[[1]])
    }
  z
}
  <environment: namespace:e1071>
  Then I beleive I have to add up the hamming matrices into one matrix 
  """

  #Data preparation for output file - transposing matrices 
  tincidence <- t(incidence) 
  tthresholded <- t(thresholded)
  
  #Manipulating the fileName to be able to create a new file w/ a meaningful file name
  #I may be able to split by / then edit the paste below f[1] + f[2] + f[3] + f[4] + f[5]
  f <- strsplit(file, split='.', fixed=TRUE)
  f <- unlist(f)
  
  #This will not work -> because you end up 
  #Setting the current directory for the output files to be placed, and never 
  #setwd("/home/zackymo/Desktop/birth_data1_out_files")
  
  #Creating the various output files 
  write(tincidence, file = paste(f[1],"toInc", sep = ""),
        ncolumns = ncol(df),
        sep = " ") 
  
  write(tthresholded, file = paste(f[1],"toThresholded", sep = ""),
        ncolumns = ncol(df),
        sep = " ") 

  #write(hamming, file = paste(f[1],"toHamming", sep = ""),
   #     ncolumns = ncol(df),
    #    sep = " ")
  
  #now write a program to add up the hamming distance files 
}


